...
def plotRawLFPTraces(data, **kwargs):
	...
	for i, chann in enumerate(channs):
		test_above_thres = np.ravel(np.nonzero(np.greater(data[:,chann],mean_vec[i] + 10*std_vec[i])))
		test_below_thres = np.ravel(np.nonzero(np.less(data[:,chann],mean_vec[i] - 10*std_vec[i])))
		ind_to_be_corrected = np.append(test_above_thres, test_below_thres)
		ind_to_be_corrected = [ind for ind in ind_to_be_corrected]
		print len(ind_to_be_corrected)
	...
	mean_vec = np.mean(data[:][channs], axis = 0)
	std_vec = np.std(data[:][channs], axis = 0)
	trace_dist = 1.5*np.max([std_vec])  # don't include std of time stamps when 
	...
	cmap = mpl.cm.brg
	for i, chann in enumerate(channs):
		plt.plot(times,data[:][chann] - mean_vec[i] + i*trace_dist,color=cmap(i/float(len(channs))))
	plt.xlabel('Time (s)')
