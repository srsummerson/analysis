...
def probabilisticFreeChoicePilotTask_logisticRegression(reward1, target1, trial1, reward3, target3, trial3, stim_trials):
    ...
    return model_block1, model_block3, y_block1_score, y_block3_score
...
'''
1. look at decsion rule and classification for trials: does not equal null model  
2. do regression for not separate regressors: better results
3. repeat analysis (w/o stim history) for first trial following stimulation: see first _firstTrial.py
4. can we predict if he was rewarded on the previous trial given his choice?  are there false alarms for positive predicts when he actually got stim instead of reward?
5. what would the roc auc be for the null hypothesis?
'''
