...
def free_choice_behavior_analysis(hdf_file):
	...
	for i in range(0,target_freechoice.size):
		...
		prob_choose_high_freechioce[i] = sum(chosen_high_freechoice)/numpy.minimum(i+1,running_avg_length)
		prob_choose_low_freechoice[i] = sum(chosen_low_freechoice)/numpy.minimum(i+1,running_avg_lengh)
	...
	for i in range(0,target_all.size):
		...
		prob_choose_high_all[i] = sum(chosen_high_all)/numpy.minimum(i+1,running_avg_length)
		prob_choose_low_all[i] = sum(chosen_low_all)/numpy.minimum(i+1,running_avg_lengh)
