...
plt.xlabel("Normalized change in Firing Rate: (A' - A)/A")
...
plt.text(0.5, 0.225, 'p = %0.2f (t-test)' % (p_cd_m))
plt.text(2.5, 0.175, 'p = %0.2f (t-test)' % (p_acc_m))
...
plt.legend()
plt.xticks([0.5, 2.5], ['Cd', 'ACC'])
plt.text(0.5, 0.65, 'p = %0.2f (t-test)' % (p_cd_l))
plt.text(2.5, 0.5, 'p = %0.2f (t-test)' % (p_acc_l))
plt.ylabel("Normalized change in Firing Rate: (A' - A)/A")
...
# mann-whitney
mw_cd_m, mw_p_cd_m = stats.mannwhitneyu(data_m[0], data_m[1])
mw_acc_m, mw_p_acc_m = stats.mannwhitneyu(data_m[2], data_m[3])
mw_cd_l, mw_p_cd_l = stats.mannwhitneyu(data_l[0], data_l[1])
mw_acc_l, mw_p_acc_l = stats.mannwhitneyu(data_l[2], data_l[3])

print "Mann-Whitney - Mario - Cd - p: %f" % (mw_p_cd_m)
print "Mann-Whitney - Mario - ACC - p: %f" % (mw_p_acc_m)
print "Mann-Whitney - Luigi - Cd - p: %f" % (mw_p_cd_l)
print "Mann-Whitney - Luigi - ACC - p: %f" % (mw_p_acc_l)

kw_cd_m, kw_p_cd_m = stats.kruskal(data_m[0], data_m[1])
kw_acc_m, kw_p_acc_m = stats.kruskal(data_m[2], data_m[3])
kw_cd_l, kw_p_cd_l = stats.kruskal(data_l[0], data_l[1])
kw_acc_l, kw_p_acc_l = stats.kruskal(data_l[2], data_l[3])

print "Kruskal-Wallis - Mario - Cd - p: %f" % (kw_p_cd_m)
print "Kruskal-Wallis - Mario - ACC - p: %f" % (kw_p_acc_m)
print "Kruskal-Wallis - Luigi - Cd - p: %f" % (kw_p_cd_l)
print "Kruskal-Wallis - Luigi - ACC - p: %f" % (kw_p_acc_l)


plt.figure()
plt.subplot(121)
plt.boxplot(data_m, 0, '')
plt.text(1.5, 0.7, 'p = %0.2f (Mann-Whitney)' % (mw_p_cd_m))
plt.text(3.5, 0.7, 'p = %0.2f (Mann-Whitney)' % (mw_p_acc_m))
plt.title('Mario')
plt.ylabel("Normalized change in Firing Rate: (A' - A)/A")
plt.xticks([1, 2, 3, 4], ['Cd-Sham', 'Cd-Stim', 'ACC-Sham', 'ACC-Stim'])
plt.subplot(122)
plt.boxplot(data_l, 0, '')
plt.title('Luigi')
plt.text(1.5, 1.25, 'p = %0.2f (Mann-Whitney)' % (mw_p_cd_l))
plt.text(3.5, 1.25, 'p = %0.2f (Mann-Whitney)' % (mw_p_acc_l))
plt.ylabel("Normalized change in Firing Rate: (A' - A)/A")
plt.xticks([1, 2, 3, 4], ['Cd-Sham', 'Cd-Stim', 'ACC-Sham', 'ACC-Stim'])
