...
def FreeChoiceTaskPerformance(hdf_file):
	...
	#plt.subplot(211)
	#plt.figure()
	plt.plot(range(1,target_freechoice.size+1),prob_choose_high_freechoice,'c',label='High-value target')
	plt.plot(range(1,target_freechoice.size+1),prob_choose_low_freechoice,'m',label='Low-value target')
	plt.axis([1,target_freechoice.size,0,1])
	plt.axis([1,target_freechoice.size, -0.5, 1.5])
	plt.plot(chose_high,0.2*chose_high_reward+1.1,'c|')
	plt.plot(chose_low,-0.2*chose_low_reward-0.1,'m|')
	plt.xlabel('Trials')
	plt.ylabel('Probability of Target Selection')
	...
	plt.subplot(212)
	plt.plot(range(1,target_freechoice.size+1),prob_reward_high_freechoice,'c',label='High-value target')
	plt.plot(range(1,target_freechoice.size+1),prob_reward_low_freechoice,'m',label='Low-value target')
	plt.axis([1,target_freechoice.size, 0, 1])
	plt.xlabel('Trials')
	plt.ylabel('Probability of Reward')
	plt.legend()
...
def PeriStimulusFreeChoiceBehavior(hdf_file, ntrials):
	reward1, target1, instructed_or_freechoice_block1, target_side1, reward3, target3, instructed_or_freechoice_block3, target_side3, stim_trials = FreeChoicePilotTask_Behavior(hdf_file)
	...
	number_aligned_choices_rewarded = np.zeros(ntrials)
	number_aligned_choices_unrewarded = np.zeros(ntrials)
	counter_rewarded = 0
	...
	prob_choose_low_aligned_rewarded = prob_choose_low_aligned_rewarded/np.max([np.ones(ntrials), number_aligned_choices_rewarded],axis=0) # conditional probability: p(choose low | rewarded)
...
def FreeChoiceBehavior_withStressTrials_CLStim(hdf_file):
	hdf = tables.openFile(hdf_file)
