...
def PopulationResponse(filename,*args):
	...
	for block in block_num:
		...
		sig_population_presma = (sig_population_presma > 0.95*np.ones(len(sig_population_presma)))
		sig_population_sma = (sig_population_sma > 0.95*np.ones(len(sig_population_sma)))
		sig_population_m1 = (sig_population_m1 > 0.95*np.ones(len(sig_population_m1)))
		sig_population_pmd = (sig_population_pmd > 0.95*np.ones(len(sig_population_pmd)))
