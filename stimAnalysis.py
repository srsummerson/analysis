		...
		for train in spiketrains:
			...
			if (train.name[-5:]!='Code0')&(train.name[-6:0]!='Code31'):
				...
				if (channel in sma):
					...
					plt.fill_between(time,averages_zscored-std_scored,averages_zscored+std_scored,facecolor='gray',alpha=0.5,linewidth=1.0)
					...
					plt.fill_between(range(0,len(avg_waveform)),avg_waveform-sem_waveform,avg_waveform+sem_waveform,facecolor='gray',alpha=0.5,linewidth=1.0)
					plt.tight_layout()
