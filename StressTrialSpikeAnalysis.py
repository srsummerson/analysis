...
plt.figure()
plt.subplot(1,2,1)
plt.bar(ind_stress, spikerates_stress, color = 'y', yerr = spikerates_sem_stress, label='Stress')
plt.xticks(ind_stress, labels_stress)
plt.xlabel('Units')
plt.ylabel('Avg Firing Rate (Hz)')
plt.subplot(1,2,2)
plt.bar(ind_reg, spikerates_reg, color = 'y', yerr = spikerates_sem_reg, label='Stress')
