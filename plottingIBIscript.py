...
def plottingIBI(IBI,tankname,block_num):
	...
	IBI_hist_first = np.histogram(IBI_first,bins=hist_bins)
	IBI_hist_second = np.histogram(IBI_second,bins=hist_bins)
	IBI_hist_third = np.histogram(IBI_third,bins=hist_bins)
